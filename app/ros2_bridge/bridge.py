...
class ROS2Bridge:
    ...
    def _setup_vr_subscribers(self):
        ...
        # 左手Joy (摇杆、trigger、grip、buttons)
        try:
            def left_joy_callback(msg: Joy):
                # axes: [joystick_x, joystick_y, trigger, grip]
                if len(msg.axes) >= 2:
                ...
                if len(msg.axes) >= 4:
                    ...
                    # Clutch 由 grip 值决定 (grip > 0.8 视为 clutch engaged)
                    self.vr_state['left_clutch_engaged'] = msg.axes[3] > 0.8
                # buttons: [A, B, X, Y, Menu, JoyClick]
